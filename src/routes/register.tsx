...
function MachakosFlow() {
  ...
  async function onContinue() {
    ...
    try {
      ...
      if (step === "pay") {
        ...
        if (!Number.isFinite(n) || n <= 0) { setError("Enter the amount you sent."); return; }
        if (n > balance) {
  ...
  function onBack() {
    setError(null);
    if (step === "entry" && entryIdx > 0) { setEntryIdx(entryIdx - 1); return; }
    if (step === "pay") { setEntryIdx(Math.max(0, pickedSwimmers.length - 1)); setStep("entry"); return; }
        ...
        <div className="ng-panel mt-4 p-6 sm:p-7">
            ...
            <>
              ...
              {swimmers.length === 0 ? (
                <p className="rounded-xl bg-white/[.06] p-4 text-[13.5px] leading-relaxed text-white/70">
                  ...
                  <a href="/welcome" className="font-semibold text-[var(--ng-cyan)] underline-offset-4 hover:underline">
                    registration page
                  </a>{" "}
                  first.
                </p>
          ...
          {step === "details" && (
            <>
              ...
              <label className="ng-label" htmlFor="r-name">Your full name</label>
              <input id="r-name" className="ng-field" value={pName}
                     autoComplete="name"
                     onChange={(e) => setPName(e.target.value)} />

              <label className="ng-label mt-4" htmlFor="r-phone">Phone number</label>
              <input id="r-phone" className="ng-field" value={pPhone} inputMode="tel"
              ...
              <div className="mt-1.5 flex gap-2">
                {(["Female", "Male"] as const).map((g) => (
                  <button key={g} type="button" onClick={() => setPGender(g)}
              ...
              <div className="mt-1.5 flex flex-wrap gap-2">
                {(["Yes", "No", "Yet to decide"] as const).map((v) => (
                  <button key={v} type="button" onClick={() => setPSleepover(v)}
            ...
            <>
              <p className="mt-1.5 text-[14px] leading-relaxed text-white/65">
                ...
                {pickedSwimmers.length > 1 && (
                  <span className="text-white/50"> · swimmer {entryIdx + 1} of {pickedSwimmers.length}</span>
                )}
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="ng-label" htmlFor="r-age">Age at the event</label>
                  <input id="r-age" className="ng-field" type="number" min={4} max={25}
                         inputMode="numeric" value={form.age}
              ...
              <span className="ng-label mt-5 block">Does {current.name.split(" ")[0]} carry a phone?</span>
              <div className="mt-1.5 flex gap-2">
                ...
                {([
                  ...
                  <div key={k}>
                    <label className="ng-label" htmlFor={`r-${k}`}>
                      {label} <span className="font-normal normal-case tracking-normal text-white/40">optional</span>
                    </label>
                    <textarea id={`r-${k}`} className="ng-field" rows={2} value={form[k]}
            ...
            receipt ? (
              <>
                <p className="mt-2 rounded-xl border border-[var(--ng-electric)]/40 bg-[var(--ng-electric)]/10 p-4 text-[14px] leading-relaxed">
                  Recorded: <span className="font-semibold">{formatKes(receipt.amount)}</span>, reference{" "}
                  <span className="font-mono font-semibold">{receipt.reference}</span>. Keep the
                  M-Pesa message — it is the proof, this is only the record.
                </p>
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  {balance > 0 && (
                    <button type="button" className="ng-btn ng-btn-ghost"
                            onClick={() => { setReceipt(null); setAmount(String(balance)); }}>
                      Record another payment
                    </button>
                  )}
                  <a href="/tracker" className="ng-btn ng-btn-primary">Back to Analytics</a>
                </div>
              </>
            ) : (
              <>
                ...
                {balance > 0 ? (
                  <>
                    ...
                    <div className="mt-5 grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="ng-label" htmlFor="r-amount">Amount sent (KES)</label>
                        <input id="r-amount" className="ng-field" type="number" inputMode="numeric"
                               min={1} max={balance} value={amount}
                               onChange={(e) => setAmount(e.target.value)} />
                      </div>
                      <div>
                        <label className="ng-label" htmlFor="r-ref">M-Pesa reference</label>
                        <input id="r-ref" className="ng-field" value={reference}
